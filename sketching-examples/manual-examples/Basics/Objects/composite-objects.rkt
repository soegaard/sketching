...
(class Ring (x y diameter on)
  ; x,y       x- and y-coordiante
...
(class Egg (x y tilt angle scalar)
  ...
  ;; Constructor
  (:= angle 0.)    ; default value for floats in P are 0.0
  (/= scalar 100.)
...
(class EggRing (ovoid circle) ; fields
  #:init (x y t sp)           ; arguments for the constructor
  ;; Constructor
  (:= circle (new Ring))
